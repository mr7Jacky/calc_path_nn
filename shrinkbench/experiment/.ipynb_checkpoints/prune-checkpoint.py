...
from ..metrics import model_size, flops, accuracy, class_accuracy
...
class PruningExperiment(TrainingExperiment):
    ...
    def pruning_metrics(self):
        ...
        ## Code to run class specific metrics
        self.class_metrics()

        return metrics
    
    def class_metrics(self):
        model_acc = accuracy(self.model, self.val_dl)
        class_acc = class_accuracy(self.model, self.val_dl)
