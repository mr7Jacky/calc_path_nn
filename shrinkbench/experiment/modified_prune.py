...
class PruningClass(TrainingExperiment):
    ...
    def load_model(self, file_name=None, checkpoint=False, prune=False):
        ...
        if file_name is not None and not checkpoint:
            ...
            checkpoint = torch.load(f"{self.path}/checkpoints/checkpoint.pt", map_location=torch.device(device))
        elif prune:
            checkpoint = torch.load(f"{self.path}/checkpoints/checkpoint-{self.compression}.pt", map_location=torch.device(device))
