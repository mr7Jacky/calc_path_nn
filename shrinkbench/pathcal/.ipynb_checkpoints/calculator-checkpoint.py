...
def calc_flows(importances):
    ...
    for layer in reversed(list(importances.keys())):
        ...
        #print(np.amax(importances[layer]['weight']),np.amin(importances[layer]['weight']))
...
def cal_linear_flows(prev_flows, weights, threshold=0):
    ...
    if prev_flows is None:
        ...
        next_flows = np.tile(prev_flows, (weights.shape[1],)).reshape(weights.shape) * weights # Change here to swap between add and multiply
...
def cal_conv_flows(prev_flows, weights, threshold=0):
    ...
    """
    if prev_flows is not None:
        new_weights = weights * prev_flows[:, None, None, None] # Change here to swap between add and multiply
    else:
        new_weights = weights
